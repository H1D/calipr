...
```toon
decisions[15]{id,date,title,status}:
  ...
  015,2026-02-28,GitHub Pages deployment with custom domain,accepted
```
...
## ADR-015: GitHub Pages deployment with custom domain

**Status**: accepted
**Date**: 2026-02-28
**Context**: Project hosted on Netlify but moving to GitHub Pages for simpler CI/CD integration.
**Decision**: GitHub Actions workflow builds with Bun + Vite on push to main, deploys via `actions/deploy-pages@v4`. Custom domain `calipr.artems.net` via Cloudflare CNAME (proxied) + GitHub Pages CNAME file in `public/`.
**Consequences**: Zero-config deploys on push. Cloudflare proxy provides CDN caching and DDoS protection on top of GitHub Pages SSL.
